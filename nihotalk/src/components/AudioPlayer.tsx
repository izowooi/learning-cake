...
export function AudioPlayer({ text, onTimeUpdate }: AudioPlayerProps) {
  const [accent, setAccent] = useState<TTSAccent>('female')
  ...
  const loadAudio = async (selectedAccent: TTSAccent) => {
    if (!text) return
  ...
  const handlePlay = async () => {
    if (!audioUrl) {
      const url = await loadAudio(accent)
      if (!url) return
  ...
  const handleAccentChange = async (newAccent: TTSAccent) => {
    ...
    // Load new audio with new accent
    if (audioRef.current) {
      audioRef.current.pause()
      audioRef.current.currentTime = 0
    }
    
    const url = await loadAudio(newAccent)
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => handleAccentChange('female')}
            ...
            <button
              onClick={() => handleAccentChange('male')}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                accent === 'male'
                  ? 'bg-primary-500 text-white'
                  : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600'
