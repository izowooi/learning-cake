...
export type DifficultyLevel = 'elementary' | 'middle_low' | 'middle_high' | 'high_school' | 'college'
...
const DIFFICULTY_OPTIONS: { value: DifficultyLevel; label: string; description: string }[] = [
  { value: 'elementary', label: '초급', description: '미국 초등학생 수준' },
  { value: 'middle_low', label: '중급 1', description: '미국 중학교 1~2학년' },
  { value: 'middle_high', label: '중급 2', description: '미국 중3~고1' },
  { value: 'high_school', label: '고급', description: '미국 고등학교 2~3학년' },
  { value: 'college', label: '최고급', description: '미국 대학생 수준' },
]

const LENGTH_OPTIONS: { value: PassageLength; label: string; words: string }[] = [
  { value: 'short', label: '짧은 글', words: '100-150 단어' },
  { value: 'medium', label: '보통', words: '200-300 단어' },
  { value: 'long', label: '긴 글', words: '400-500 단어' },
]
...
export function PassageForm({ onGenerate, isLoading }: PassageFormProps) {
  const [difficulty, setDifficulty] = useState<DifficultyLevel>('middle_low')
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {LENGTH_OPTIONS.map((option) => (
            ...
            >
              <div className="font-medium text-slate-800 dark:text-slate-200">{option.label}</div>
              <div className="text-xs text-slate-500 dark:text-slate-400">{option.words}</div>
            </button>
          ))}
        </div>
      </div>
